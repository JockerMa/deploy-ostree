...
class TestMountVar(TestCase):
    ...
    @mock.patch('deploy_ostree.steps.mount_var.run')
    def test_should_mount_with_specified_root_dir(self, run_mock: mock.Mock):
        ...
        run_mock.assert_called_once_with([
            'mount', '-o', 'bind',
            os.path.join(root_dir, 'ostree', 'deploy', 'test', 'var'),
            os.path.join(root_dir, 'ostree', 'deploy', 'test', 'deploy', 'test-deploy.0', 'var'),
        ], check=True)
    ...
    @mock.patch('deploy_ostree.steps.mount_var.run')
    def test_should_unmount_with_specified_root_dir(self, run_mock: mock.Mock):
        root_dir = os.path.join('/mnt', 'rootfs')
        cfg = Config(Source.url('url'), 'ref', stateroot='test', root_dir=root_dir)
        cfg.set_deployment_name('test-deploy.0')
